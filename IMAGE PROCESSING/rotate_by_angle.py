...
rotation_matrix_acw = cv2.getRotationMatrix2D((cols/2, rows/2), 45, 1)
rotated_image_acw = cv2.warpAffine(original_image, rotation_matrix_acw, (cols, rows))

# Display the original image and its rotations
cv2.imshow('Original Image', original_image)
cv2.imshow('Rotated 45 Degrees Clockwise', rotated_image_cw)
cv2.imshow('Rotated 45 Degrees Counterclockwise', rotated_image_acw)
cv2.waitKey(0)
